...
class FormProcessor
{
    ...
    public function process(string $formType, array $postData): bool
    {
        if ('reservation' === $formType) {
            $senderName = trim($_ENV['SENDER_NAME_RESERVE'] ?? '予約受付');
            $adminEmail = $_ENV['USER_NAME_RESERVE'] ?? 'reserve@example.com';
        } elseif ('consultation' === $formType) {
            $senderName = trim($_ENV['SENDER_NAME_CONSULTATION'] ?? '相談受付');
            $adminEmail = $_ENV['USER_NAME_CONSULTATION'] ?? 'soudan@example.com';
        }
        ...
        try {
            ...
            $adminSent = true;
            if ($adminEmail) {
                $adminSent = $this->sender->send($this->phpMailer, $adminEmail, $adminSubject, $adminBody, $userEmail, $adminBcc, '');
            }
